						...
						Children: []Widget{
							...
							PushButton{
								...
								OnClicked: func() {
									...
									go teacherdata.GetTeacher(progChan, outputFile)
								},
							},
							...
							PushButton{
								...
								OnClicked: func() {
									...
									if cmd, err := runDownloadVideoDialog(mw, fi, iconDownload); err != nil {
										...
										if fi.Append {
											var inputFile file.WorksheetFile
											...
											go syllabusvideo.AppendSyllabusVideo(progChan, fi.Year, fi.Semester, fi.Key, inputFile, outputFile)

										} else {
											go setProgressBar(progChan, mw, pb, 0, 7)
											go syllabusvideo.GetSyllabusVideo(progChan, fi.Year, fi.Semester, fi.Key, outputFile)
										}
									}
								},
							},
									...
									if cmd, err := runCalculateDifferenceDialog(mw, fi, iconCalculate); err != nil {
										...
										if fi.CalcAll {
											...
											for _, value := range files {
												...
												go difference.DifferenceCalculate(progChan, inputFile, outputFile)
											}
							...
							PushButton{
								...
								OnClicked: func() {
									...
									if cmd, err := runSplitScoreAlertDialog(mw, fi, iconSplit); err != nil {
										...
										checkOutputDir()
										var masterFile file.WorksheetFile
										var templateFile file.WorksheetFile
										var teacherFile file.WorksheetFile
										var outputFile file.WorksheetFile
										...
										go scorealert.SplitScoreAlert(progChan, masterFile, templateFile, teacherFile, outputFile)
									}
								},
							},
							VSpacer{},
							...
							PushButton{
								...
								OnClicked: func() {
									...
									if cmd, err := runMergeCourseDialog(mw, fi, iconMerge); err != nil {
										...
										go setProgressBar(progChan, mw, pb, 0, 7)
										go rapidprint.MergeRapidPrintData(progChan, inputFile, outputFile)
									}
								},
							},
								...
								OnClicked: func() {
									...
									if cmd, err := runMergeVideoDialog(mw, fi, iconMerge); err != nil {
										...
										if fi.TFile {
											...
											go setProgressBar(progChan, mw, pb, 0, 7)
											go syllabusvideo.MergeSyllabusVideoDataByList(progChan, inputFile, outputFile, teacherFile)
										} else {
											go setProgressBar(progChan, mw, pb, 0, 5)
											go syllabusvideo.MergeSyllabusVideoData(progChan, inputFile, outputFile)
										}
									}
								},
							},
...
func onOpenFileButtonClicked(owner walk.Form, filePath *walk.LineEdit, selector *walk.ComboBox) {
	...
	if ok, err := dlg.ShowOpen(owner); err != nil {
		fmt.Fprintln(os.Stderr, "logging.Error:Open master file")
		return
	} else if !ok {
...
func onBrowseFolderButtonClicked(owner walk.Form, filePath *walk.LineEdit) {
	...
	if ok, err := dlg.ShowBrowseFolder(owner); err != nil {
		fmt.Fprintln(os.Stderr, "logging.Error:Open folder")
		return
	} else if !ok {
